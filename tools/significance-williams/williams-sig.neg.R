...
for( m1 in sort( unique( unlist( a$METRIC ) ) ) ){
  ...
  cat(paste(  cor( h.scrs,  m1.scrs, method="pearson"),"\n"))
}
sink()
...
for( m1 in sort( unique( unlist( a$METRIC ) ) ) ){
  ...
  for( m2 in sort( unique( unlist( a$METRIC ) ) ) ){
    ...
    if( m1==m2 ){
      ...
      if(  cor( h.scrs,  m1.scrs, method="pearson") >  cor( h.scrs,  m2.scrs, method="pearson") ){

        p <- r.test( 
          n = N, 
  	  r12 = cor( h.scrs,  m1.scrs, method="pearson"), 
 	  r13 = cor( h.scrs,  m2.scrs, method="pearson"), 
	  r23 = cor( m1.scrs, m2.scrs, method="pearson"), 
 	  twotailed=F)$p
